...
final class AdbKeyStore {
    ...
    static AdbKeyStore loadOrCreate(Context c) throws Exception {
        ...
        writeAll(prf, p.getPrivate().getEncoded());
        writeAll(puf, p.getPublic().getEncoded());
    ...
    byte[] signAdbToken(byte[] token) throws Exception {
        ...
        byte[] di = concat(SHA1_PREFIX, token);
        int ps = len - di.length - 3;
        byte[] em = new byte[len];
        em[0] = 0; em[1] = 1;
        for (int i = 0; i < ps; i++) em[2 + i] = (byte) 0xff;
        em[2 + ps] = 0;
        System.arraycopy(di, 0, em, 3 + ps, di.length);
    ...
    byte[] adbPublicKeyPayload() {
        ...
        int words = 64;
        byte[] st = new byte[4 + 4 + words * 4 + words * 4 + 4];
        int off = 0;
        ...
        for (int i = 0; i < words; i++) {
            putLe32(st, off, n.shiftRight(i * 32).and(mask).longValue()); off += 4;
        }
        BigInteger rr = BigInteger.ONE.shiftLeft(words * 32 * 2).mod(n);
        for (int i = 0; i < words; i++) {
            putLe32(st, off, rr.shiftRight(i * 32).and(mask).longValue()); off += 4;
        }
        putLe32(st, off, k.getPublicExponent().longValue());
        return (Base64.encodeToString(st, Base64.NO_WRAP) +
                " upospa@android\0").getBytes(StandardCharsets.UTF_8);
    }
    ...
    private static byte[] toFixed(BigInteger v, int len) {
        byte[] s = v.toByteArray(), d = new byte[len];
        int c = Math.min(s.length, len);
        System.arraycopy(s, s.length - c, d, len - c, c);
        return d;
    }
